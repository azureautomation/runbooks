<#
.SYNOPSIS 
    This automation runbook stops all Azure virtual machines that are running in a subscription, resource group or single VM.
    If a tag is added to the VM, then only those VMs will be stopped if the TagName parameter is passed in.
...
.PARAMETER TagName
    Optional. The name of the Tag to look for. If specified, only VMs with this tag will be stopped.

.EXAMPLE
    Stop-AzureVM -VMResourceGroup Contoso -VM Marketing

.EXAMPLE
    Stop-AzureVM -VMResourceGroup Contoso -TagName "DevMachines"
