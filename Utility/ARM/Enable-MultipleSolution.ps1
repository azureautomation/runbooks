...
try
{
    ...
    if (([string]::IsNullOrEmpty($AutomationResourceGroupName)) -or ([string]::IsNullOrEmpty($AutomationAccountName)))
    {
        ...
        $AutomationResource = Get-AzureRMResource -ResourceType Microsoft.Automation/AutomationAccounts -AzureRmContext $SubscriptionContext -ErrorAction Stop

        foreach ($Automation in $AutomationResource)
        {
            $Job = Get-AzureRMAutomationJob -ResourceGroupName $Automation.ResourceGroupName -AutomationAccountName $Automation.Name -Id $PSPrivateMetadata.JobId.Guid -AzureRmContext $SubscriptionContext -ErrorAction SilentlyContinue
    ...
    $EnableSolutionRunbook = Get-AzureRmAutomationRunbook -ResourceGroupName $AutomationResourceGroupName `
    ...
    if ($EnableSolutionRunbook.State -ne "Published" -and $EnableSolutionRunbook.State -ne "Edit")
    {
        ...
        Import-AzureRmAutomationRunbook -ResourceGroupName $AutomationResourceGroupName `
            -AutomationAccountName $AutomationAccountName -Path $LocalFolder\$DependencyRunbookName.ps1 `
            -Published -Type PowerShell -AzureRmContext $SubscriptionContext -Force -ErrorAction stop | Write-Verbose
    ...
    if ( -not [string]::IsNullOrEmpty($VMResourceGroupName) -and -not [string]::IsNullOrEmpty($VMName) )
    {
        ...
        $VMList = Get-AzureRMVM -ResourceGroupName $VMResourceGroupName -Name $VMName -AzureRmContext $NewVMSubscriptionContext `
    ...
    elseif ( -not [string]::IsNullOrEmpty($VMResourceGroupName) )
    {
        ...
        $VMList = Get-AzureRMVM -ResourceGroupName $VMResourceGroupName -AzureRmContext $NewVMSubscriptionContext `
    ...
    {
        $ActiveJob = Get-AzureRMAutomationJob -ResourceGroupName $AutomationResourceGroupName `
    ...
    {
        $OutputJob = Get-AzureRmAutomationJobOutput -ResourceGroupName $AutomationResourceGroupName `
        ...
        {
            (Get-AzureRmAutomationJobOutputRecord -ResourceGroupName $AutomationResourceGroupName `
        ...
        $ErrorJob = Get-AzureRmAutomationJobOutput -ResourceGroupName $AutomationResourceGroupName `
            -AutomationAccountName $AutomationAccountName -Id `
        ...
        $WarningJob = Get-AzureRmAutomationJobOutput -ResourceGroupName $AutomationResourceGroupName `
            -AutomationAccountName $AutomationAccountName -Id `
